...
def gemm(gemm_kernel, A, B, C, A_rows, A_cols, B_cols):
    ...
    # better too many blocks than too little
    grid = (B_cols / block_size + 1, A_rows / block_size + 1)
    gemm_kernel(A, B, C, np.intc(A_rows), np.intc(A_cols),
                np.intc(B_cols), block=block, grid=grid)
...
def concatenation_rows(concatenate_rows_kernel, A, A_conc, A_rows, A_cols):
    ...
    concatenate_rows_kernel(A, A_conc, np.intc(
        A_rows * A_cols / 32), block=block, grid=grid)
...
def concatenation_cols(concatenate_cols_kernel, A, A_conc, A_rows, A_cols):
    ...
    concatenate_cols_kernel(A, A_conc, np.intc(
        A_rows), np.intc(A_cols), block=block, grid=grid)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--n', help='matrix dimension (square)', type=int, default=2048)
    parser.add_argument(
    ...
    for i in xrange(N_RUNS):
        ...
        print(" execution_time = " + str(xnor_timings[i]) + "s")
        ...
        print(" execution_time = " + str(base_timings[i]) + "s")
    ...
    print "Avg Base kernel execution time over %d runs: %f +/- %f" % ((N_RUNS, base_timings.mean(), base_timings.std()))
